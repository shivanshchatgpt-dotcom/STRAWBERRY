...
pub fn generate(title: &str, raw: &str) -> GeneratedBrief {
    ...
    // Negative knowledge: "what" and "why" joined so one row survives a copy.
    for rj in &ex.rejected {
        let content = match &rj.why {
            Some(why) => format!("{} — why: {}", rj.what, why),
            None => rj.what.clone(),
        };
        artifacts.push(("rejected".to_string(), content));
    }
    for c in &ex.constraints {
        artifacts.push(("constraint".to_string(), c.clone()));
    }
    for id in &ex.identifiers {
        artifacts.push(("identifier".to_string(), id.tagged()));
    }
    ...
    if !ex.questions.is_empty() {
        md.push_str("\n## Questions\n");
        for q in ex.questions.iter().take(crate::rules::MAX_QUESTIONS) {
            md.push_str("- ");
            md.push_str(q);
            md.push('\n');
        }
    ...
    if !ex.key_points.is_empty() || !ex.file_paths.is_empty() {
        md.push_str("\n## Answers / Key Points\n");
        for p in ex.key_points.iter().take(crate::rules::MAX_KEY_POINTS) {
            md.push_str("- ");
            md.push_str(p);
            md.push('\n');
        }
    ...
    if !ex.code_blocks.is_empty() {
        ...
        for (i, cb) in ex
            .code_blocks
            .iter()
            .take(crate::rules::MAX_CODE_BLOCKS)
            .enumerate()
        {
            if i > 0 {
                md.push('\n');
    ...
    if !ex.commands.is_empty() {
        md.push_str("\n## Commands\n");
        for c in ex.commands.iter().take(crate::rules::MAX_COMMANDS) {
            md.push_str("- `");
            md.push_str(c);
            md.push_str("`\n");
        }
    ...
    if !ex.errors.is_empty() {
        md.push_str("\n## Errors\n");
        for e in ex.errors.iter().take(crate::rules::MAX_ERRORS) {
            md.push_str("- ");
            md.push_str(e);
            md.push('\n');
        }
    ...
    if !ex.decisions.is_empty() {
        md.push_str("\n## Decisions\n");
        for d in ex.decisions.iter().take(crate::rules::MAX_DECISIONS) {
            md.push_str("- ");
            md.push_str(d);
            md.push('\n');
        }
    ...
    // Rejected sits directly after decisions: the pair is what prevents a
    // receiving reader from re-proposing a dead end.
    if !ex.rejected.is_empty() {
        md.push_str("\n## Rejected / Already Tried\n");
        for rj in ex.rejected.iter().take(crate::rules::MAX_REJECTED) {
            md.push_str("- ");
            md.push_str(&rj.what);
            if let Some(why) = &rj.why {
                md.push_str(" — why: ");
                md.push_str(why);
            }
            md.push('\n');
    ...
    if !ex.constraints.is_empty() {
        md.push_str("\n## Constraints\n");
        for c in ex.constraints.iter().take(crate::rules::MAX_CONSTRAINTS) {
            md.push_str("- ");
            md.push_str(c);
            md.push('\n');
        }
    ...
    if !ex.identifiers.is_empty() {
        md.push_str("\n## Identifiers\n");
        for id in ex.identifiers.iter().take(crate::rules::MAX_IDENTIFIERS) {
            md.push_str("- `");
            md.push_str(&id.tagged());
            md.push_str("`\n");
        }
    }
    ...
    if !ex.action_items.is_empty() {
        md.push_str("\n## Action Items\n");
        for a in ex.action_items.iter().take(crate::rules::MAX_ACTION_ITEMS) {
            md.push_str("- ");
            md.push_str(a);
            md.push('\n');
        }
    ...
    if !ex.truncated.is_empty() {
        md.push_str("\n> ");
        md.push_str(crate::rules::MORE_NOTE);
        md.push_str(if ex.truncated.len() > 1 {
            " (multiple sections)"
        } else {
            ""
        });
    ...
    md.push_str("\n## Metadata\n");
    md.push_str("- source: manual/rule-based extraction\n");
    md.push_str(&format!(
        "- char count: {}\n- word count: {}\n",
...
mod tests {
    ...
    fn empty_sections_omitted() {
        ...
        assert!(!g.markdown.contains("## Rejected"));
        assert!(!g.markdown.contains("## Identifiers"));
    }
    ...
    fn stats_are_accurate() {
        ...
        assert_eq!(g.stats.word_count, 5);
    ...
    fn limits_add_truncation_note() {
        ...
        let dec_artifacts = g.artifacts.iter().filter(|(k, _)| k == "decision").count();
        assert_eq!(dec_artifacts, crate::rules::MAX_DECISIONS);
    }

    #[test]
    fn rejected_section_and_artifacts_present() {
        let raw = "We tried sqlx but it didn't work because of async overhead.";
        let g = generate("Rej", raw);
        assert!(g.markdown.contains("## Rejected / Already Tried"));
        assert!(g.markdown.contains("why: of async overhead"));
        let rejected: Vec<&(String, String)> =
            g.artifacts.iter().filter(|(k, _)| k == "rejected").collect();
        assert_eq!(rejected.len(), 1);
        assert!(rejected[0].1.contains("why:"));
    }

    #[test]
    fn identifier_artifacts_present() {
        let g = generate("Ids", "Bind localhost:1420 and read HERMES_API_KEY");
        let ids: Vec<&String> = g
            ...
            .map(|(_, v)| v)
            .collect();
        assert!(ids.iter().any(|v| v.as_str() == "port:1420"));
        assert!(ids.iter().any(|v| v.as_str() == "env:HERMES_API_KEY"));
    }

    #[test]
    fn constraint_artifacts_present() {
        let g = generate("C", "The daemon must never call the network.");
        assert!(g.artifacts.iter().any(|(k, _)| k == "constraint"));
        assert!(g.markdown.contains("## Constraints"));
    }
}
